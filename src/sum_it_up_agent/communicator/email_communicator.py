...
class EmailCommunicator(ICommunicator):
    ...
    def send(self, request: CommunicationRequest) -> Dict[str, Any]:
        ...
        # html = self.render_email_html(summary, title=request.subject)
    ...
    @staticmethod
    def render_email_html(summary_text: str, title: Optional[str] = None) -> str:
        """
        Minimal renderer: the summary is a plain string generated by an LLM.
        - Escapes HTML by default (safe).
        - Preserves line breaks.
        """
        safe = escape((summary_text or "").strip()).replace("\n", "<br/>")
        header = escape((title or "Meeting Summary").strip())
        return f"""\
        <!doctype html>
        <html>
